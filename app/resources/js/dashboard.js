...
const tableDefinitions = {
    ...
    activity: {
        ...
        filterRow: (row, filter) => {
            ...
            if (filter === 'login') {
                return String(row.action).startsWith('Login');
            }
            if (filter === 'ubah') {
                const action = String(row.action);
                return action.startsWith('Tambah Data') || action.startsWith('Edit Data') || action.startsWith('Hapus Data');
            }
            return String(row.action).startsWith('Export Laporan');
        },
...
async function submitEntityForm() {
    ...
    if (state.modalState.mode === 'add') {
        ...
        if (table === 'stock') {
            fieldToCheck = 'code';
            let candidateCode = String(payload.code || '').trim().toUpperCase();
            if (!candidateCode && payload.name) {
                const namePart = String(payload.name).trim().toUpperCase().replace(/[^A-Z0-9]+/g, '') || 'BARANG';
                candidateCode = ('PRD-' + namePart).substring(0, 45);
            }
            newValue = candidateCode;
            duplicateMessage = `Kode barang (${newValue}) sudah ada, yakin ingin menambahkan? (Sistem akan otomatis menambah angka di belakangnya)`;
        } else {
            fieldToCheck = payload.name !== undefined ? 'name' : (payload.title !== undefined ? 'title' : null);
            if (fieldToCheck) {
                newValue = String(payload[fieldToCheck]).trim().toLowerCase();
                duplicateMessage = 'Data dengan nama tersebut sudah ada, yakin ingin menambahkan?';
            }
        }

        if (fieldToCheck && newValue) {
            const isDuplicate = (state.data[table] || []).some(row => {
                if (!row[fieldToCheck]) return false;
                if (table === 'stock') {
                    return String(row[fieldToCheck]).trim().toUpperCase() === newValue;
                }
                return String(row[fieldToCheck]).trim().toLowerCase() === newValue;
            });
...
function filterPendingDeletedData(table, dataArray) {
    if (state.undoDelete && state.undoDelete.item && state.undoDelete.item.table === table && Array.isArray(dataArray)) {
        const pendingId = Number(state.undoDelete.item.id);
        return dataArray.filter(row => Number(row.id) !== pendingId);
    }
    return dataArray;
}
...
async function refreshCustomersFromBackend() {
    ...
    try {
        ...
        state.data.customers = filterPendingDeletedData('customers', result.data);
...
async function refreshCalendarEventsFromBackend() {
    ...
    try {
        ...
        state.data.calendarEvents = filterPendingDeletedData('calendarEvents', result.data);
    } catch (error) {
...
async function refreshStockFromBackend() {
    ...
    try {
        ...
        state.data.stock = filterPendingDeletedData('stock', result.data);
...
async function refreshFinanceFromBackend() {
    ...
    try {
        ...
        state.data.finance = filterPendingDeletedData('finance', result.data);
...
async function pollAllData() {
    ...
    const fetchPromises = tablesToPoll.map(async (table) => {
        try {
            ...
            return { table, data: filterPendingDeletedData(table, result.data) };
        } catch (error) {
            return null;
        }
    });
